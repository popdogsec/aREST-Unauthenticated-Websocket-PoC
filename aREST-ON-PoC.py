...
#
# This Script Is Capable Of Enabling All Pins On An ESP8266 MicroController
...
if OK == 'n' or OK == 'N':
    ...
    if RETURN_MESSAGE.decode('utf-8')[56] != 'f' and RETURN_MESSAGE.decode('utf-8')[56] != 'e':
        ...
        while index < len(PIN_STATUS):
            ...
            RETURN_MESSAGE = r.content
            if RETURN_MESSAGE.decode('utf-8')[16] == "1":
        ...
        while index < len(PIN_STATUS):
             if PIN_STATUS[index] == True:
                 print( "Pin " + str(index) + " Is Already Enabled, This Pin Will Be Skipped")
             else:
                 print("Pin " + str(index) + " Is Marked To Be Enabled")
             index = index + 1
        ...
        PIN_NO = 0
        index = 0
        TARGET_URI = AREST_URI + USER + DIGITAL + str(PIN_NO) + ENABLE
        while index < len(PIN_STATUS):
            if PIN_STATUS[index] == False:
                r = requests.get(TARGET_URI)
                print("Enabling Pin " + str(index))
            index = index + 1
            PIN_NO = PIN_NO + 1
            TARGET_URI = AREST_URI + USER + DIGITAL + str(PIN_NO) + ENABLE
        ...
        # example enable message for pin 4 without escape characters: ["{\"msg\":\"method\",\"method\":\"digitalWrite\",\"params\":[\"272925\",4,1],\"id\":\"0\"}"]
        ENABLE_MESSAGE = '[\"{\\\"msg\\\":\\\"method\\\",\\\"method\\\":\\\"digitalWrite\\\",\\\"params\\\":[\\\"' + USER + '\\\",' + str(PIN_NO) + ',1],\\\"id\\\":\\\"0\\\"}"]'
